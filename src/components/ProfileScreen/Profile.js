...
import {getProfileById, updateCurrentProfile} from "../../services/userService";
        ...
        return (
            <>
                <div className="profile-name">
                    <h4 className="name">
                        {user.firstName} {user.lastName} {provider !== null && provider.verified
                                                          === true && <i
                ...
                {!isFriendProfile &&
                 <>
                     <button className="btn btn-light edit"
                             onClick={() => openEditProfile()}>Edit profile
                     </button>
                            ...
                            <ul className="nav nav-tabs mt-2">
                                <li className="nav-item">
                                    <div aria-current="page"
                                         className={`nav-link wd-profile-tab ${active === "feed"
                                                                               ? "active" : ""}`}
                                         onClick={() => setActive("feed")}>
                                        Feed
                                    </div>
                                </li>
                                {!isFriendProfile && (user.role === "user" || (provider !== null
                                                                               && provider.verified
                                                                               === true)) && <li
                                     className="nav-item">
                                     <div
                                         className={`nav-link wd-profile-tab ${active === "add_post"
                                                                               ? "active" : ""}`}
                                ...
                                {user.role === "provider" &&
                                 <li className="nav-item">
                                     <div className={`nav-link ${active === "services" ? "active"
                                                                                       : ""}`}
                                ...
                                <li className="nav-item">
                                    <Link className={`nav-link ${active === "my_map" ? "active"
                                                                                     : ""}`}
                                          to={{
                                              pathname: `/mymap`,
                                ...
                                <li className="nav-item">
                                    <Link className={`nav-link ${active === "super_map" ? "active"
                                                                                        : ""}`}
                                          to={`/super`} exact={true}>
                                        Super Map
                                    </Link>
...
const EditProfile = ({user}) => {
    ...
    return (
        <div id="wd-edit-profile-form" className="shadow wd-form-popup pt-0 m-2 p-2">
            <div className="row mb-2 wd-edit-header pb-2">
                <div className="col-1 mt-3" onClick={() => {
                    closeEditProfile()
                }}><i className="fas fa-times"/></div>
                <div className="col-9 wd-profile-name mt-3">Edit profile
                </div>
                <div className="col-2 mt-2">
                    <button className="btn wd-profile-save rounded-pill"
                            onClick={handleSaveProfile}>Save
                    </button>
                </div>
            </div>
            ...
            <div className="form-floating mb-3">
                <textarea className="form-control" value={bio}
                          id="floatingTextarea" onChange={bioChangeHandler}/>
